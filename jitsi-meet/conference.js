...
export default {
    ...
    _setupListeners() {
        ...
        room.on(JitsiConferenceEvents.USER_JOINED, (id, user) => {
            ...
            console.log('JITSI CONFERENCEE USISNSNGG JOINED')
        ...
        console.log('Inside the state remote audio')
        const conference = APP.conference._room;

        conference.getParticipants().forEach(participant => {
            const audioTrack = participant.getTracks().find(t => t.getType() === 'audio');

            if (!audioTrack) {
                console.warn(`No audio track for participant ${participant.getId()}`);
                return;
            }
            ...
            const socket = new WebSocket("ws://localhost:2700"); // Vosk server
            socket.binaryType = "arraybuffer";

            socket.onopen = () => {
                console.log("WebSocket to Vosk opened");
        ...
        room.on(JitsiConferenceEvents.USER_ROLE_CHANGED, (id, role) => {
            if (this.isLocalId(id)) {
                ...
                if (role === 'moderator') {
                    console.log("Checking if I am moderator:", APP.conference._room.isModerator());
