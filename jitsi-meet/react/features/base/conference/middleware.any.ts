...
import { startTranscription } from '../../transciption/transcriptionClient';
import {
    CONFERENCE_FAILED,
    CONFERENCE_JOINED,
...
MiddlewareRegistry.register(store => next => action => {
    ...
    const result = _conferenceJoined(store, next, action);

    const state = store.getState();
    const localAudio = state['features/base/tracks'].find(t => t.local && t.mediaType === 'audio');

    if (localAudio) {
        const jitsiTrack = localAudio.jitsiTrack;

        console.log("🎤 Starting transcription with local audio track");
        startTranscription(jitsiTrack);
    } else {
